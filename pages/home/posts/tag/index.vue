                  ...
                  <div v-if="post.tags && post.tags.length > 0" class="mb-4">
                    <div class="flex flex-wrap gap-2">
                      <NuxtLink 
                        ...
                        :class="{ 'bg-blue-100 text-blue-800': tag.slug === currentTagSlug || tag.id === currentTagSlug }"
                      >
                        #{{ tag.name }}
                      </NuxtLink>
                    </div>
...
// Computed
const currentTagSlug = computed(() => route.params.slug)
const tagName = computed(() => {
  const tag = tags.value.find(t => t.slug === currentTagSlug.value || t.id === currentTagSlug.value)
  return tag?.name || 'Tag'
})
...
const filteredPosts = computed(() => {
  let filtered = posts.value.filter(post => 
    post.tags && post.tags.some(tag => 
      tag.slug === currentTagSlug.value || tag.id === currentTagSlug.value
    )
  )

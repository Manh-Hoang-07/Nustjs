...
async function fetchAttributeValueDetails() {
  ...
  loading.value = true
  try {
    const response = await api.get(`/api/admin/attribute-values/${props.attributeValue.id}`)
...
async function fetchAttributeOptions() {
  try {
    const response = await api.get(endpoints.attributes.list, { params: { per_page: 100 } })
...
async function handleSubmit(formData) {
  try {
    ...
    await api.post(endpoints.attributeValues.update(props.attributeValue.id), dataWithMethod)
    emit('updated')
    props.onClose()
  } catch (error) {

...
async function fetchServices() {
  loading.value = true
  try {
    const response = await api.get(endpoints.shippingServices.list)
...
async function fetchProviders() {
  const response = await api.get('/api/admin/shipping/api')
...
async function handleCreateService(formData) {
  try {
    await api.post(endpoints.shippingServices.create, formData)
    await fetchServices()
    closeModal()
  } catch (error) {
...
async function handleEditService(formData) {
  try {
    ...
    await api.post(endpoints.shippingServices.update(editingService.value.id), dataWithMethod)
    await fetchServices()
    closeModal()
  } catch (error) {
    if (error.response?.status === 422 && error.response?.data?.errors) {

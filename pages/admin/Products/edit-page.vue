...
async function fetchProduct() {
  try {
    const response = await api.get(endpoints.products.update(productId.value))
...
async function fetchVariants() {
  try {
    const response = await api.get(`/api/admin/products/${productId.value}/variants`)
...
async function fetchImages() {
  try {
    const response = await api.get(`/api/admin/products/${productId.value}/images`)
...
async function handleSubmit(formData) {
  ...
  try {
    ...
    await api.post(endpoints.products.update(productId.value), dataWithMethod)
    await fetchProduct()
  } catch (error) {
...
async function deleteVariant(variant) {
  if (confirm(`Bạn có chắc chắn muốn xóa biến thể ${variant.sku}?`)) {
    try {
      await api.delete(`/api/admin/variants/${variant.id}`)
      await fetchVariants()
    } catch (error) {

        ...
        <form @submit.prevent="saveAllConfigs" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Cache driver mặc định <span class="text-red-500">*</span>
            </label>
              <p class="text-sm text-gray-500">Driver cache được sử dụng mặc định</p>
            </div>
            <div class="md:col-span-2">
            <select
              v-model="configs.default"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500': validationErrors.default }"
            >
            ...
            <div v-if="validationErrors.default" class="mt-1 text-sm text-red-600">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-1">
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Cache TTL mặc định (seconds) <span class="text-red-500">*</span>
            </label>
              <p class="text-sm text-gray-500">Thời gian sống mặc định của cache</p>
            </div>
            <div class="md:col-span-2">
            <input
              v-model.number="configs.ttl"
              type="number"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500': validationErrors.ttl }"
              placeholder="3600"
            />
...
definePageMeta({
  layout: 'admin-layout',
  title: 'Cài đặt Cache',
  description: 'Cấu hình các thông số cache',
  requiresAuth: true,
  requiresAdmin: true
})
...
const validationRules = computed(() => ({
  default: [
    { required: 'Cache driver là bắt buộc' }
  ],
  ttl: [
    { required: 'Cache TTL là bắt buộc' }
  ]
}))
...
const loadConfigs = async () => {
  ...
  try {
    const response = await apiClient.get(adminEndpoints.systemConfigs.getByGroup('cache'))

      ...
      <div class="bg-white rounded-lg shadow-md p-6">
        <form @submit.prevent="saveAllConfigs" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input
              v-model="configs.driver"
              type="text"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500': validationErrors.driver }"
              placeholder="smtp"
            />
            <div v-if="validationErrors.driver" class="mt-1 text-sm text-red-600">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input
              ...
              type="text"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500': validationErrors.host }"
              placeholder="smtp.gmail.com"
            />
            <div v-if="validationErrors.host" class="mt-1 text-sm text-red-600">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="md:col-span-2">
            <input
              v-model.number="configs.port"
              type="number"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500': validationErrors.port }"
              placeholder="587"
            />
            <div v-if="validationErrors.port" class="mt-1 text-sm text-red-600">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            />
            <div v-if="validationErrors.username" class="mt-1 text-sm text-red-600">
...
definePageMeta({
  ...
  description: 'Cấu hình các thông số gửi email',
  requiresAuth: true,
  requiresAdmin: true
})
...
const validationRules = computed(() => ({
  driver: [
    { required: 'Driver email là bắt buộc' }
  ],
  host: [
    { required: 'SMTP Host là bắt buộc' }
  ],
  port: [
    { required: 'SMTP Port là bắt buộc' }
  ],
  username: [
    { required: 'SMTP Username là bắt buộc' }
  ],
  password: [
    { required: 'SMTP Password là bắt buộc' }
  ]
}))
...
const loadConfigs = async () => {
  ...
  try {
    const response = await apiClient.get(adminEndpoints.systemConfigs.getByGroup('email'))

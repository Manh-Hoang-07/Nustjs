<template>
  <div class="p-6">
    ...
    <div v-else>
      ...
      <!-- Form tĩnh cho cấu hình notification -->
        ...
        <form @submit.prevent="saveAllConfigs" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-1">
              ...
              <p class="text-sm text-gray-500">Cho phép gửi thông báo qua email</p>
            </div>
            <div class="md:col-span-2">
              <label class="flex items-center">
                <input
                  v-model="configs.email_enabled"
                  type="checkbox"
                  class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                />
                <span class="ml-2 text-sm text-gray-700">Bật thông báo email</span>
              </label>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-1">
              ...
              <p class="text-sm text-gray-500">Cho phép gửi thông báo qua SMS</p>
            </div>
            <div class="md:col-span-2">
              <label class="flex items-center">
                <input
                  v-model="configs.sms_enabled"
                  type="checkbox"
                  class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                />
                <span class="ml-2 text-sm text-gray-700">Bật thông báo SMS</span>
              </label>
            </div>
          </div>
        </form>
...
definePageMeta({
  ...
  description: 'Cấu hình các thông số thông báo',
  requiresAuth: true,
  requiresAdmin: true
})
...
const loadConfigs = async () => {
  ...
  try {
    const response = await apiClient.get(adminEndpoints.systemConfigs.getByGroup('notification'))

        ...
        <form @submit.prevent="saveAllConfigs" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Giới hạn request per minute <span class="text-red-500">*</span>
            </label>
              <p class="text-sm text-gray-500">Số request tối đa mỗi phút</p>
            </div>
            <div class="md:col-span-2">
            <input
              v-model.number="configs.rate_limit"
              type="number"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500': validationErrors.rate_limit }"
              placeholder="60"
            />
            <div v-if="validationErrors.rate_limit" class="mt-1 text-sm text-red-600">
              {{ validationErrors.rate_limit }}
            </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-1">
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Timeout cho API requests (seconds) <span class="text-red-500">*</span>
            </label>
              <p class="text-sm text-gray-500">Thời gian timeout cho các request API</p>
            </div>
            <div class="md:col-span-2">
            <input
              v-model.number="configs.timeout"
              type="number"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500': validationErrors.timeout }"
              placeholder="30"
            />
            <div v-if="validationErrors.timeout" class="mt-1 text-sm text-red-600">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="md:col-span-1">
              ...
              <p class="text-sm text-gray-500">Cho phép CORS cho các request API</p>
            </div>
            <div class="md:col-span-2">
              <label class="flex items-center">
                <input
                  v-model="configs.cors_enabled"
                  type="checkbox"
                  class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                />
                <span class="ml-2 text-sm text-gray-700">Bật CORS</span>
              </label>
            </div>
          </div>
        </form>
...
definePageMeta({
  layout: 'admin-layout',
  title: 'Cài đặt API',
  description: 'Cấu hình các thông số API',
  requiresAuth: true,
  requiresAdmin: true
})
...
const validationRules = computed(() => ({
  rate_limit: [
    { required: 'Giới hạn request là bắt buộc' }
  ],
  timeout: [
    { required: 'Timeout là bắt buộc' }
  ]
}))
...
const loadConfigs = async () => {
  ...
  try {
    const response = await apiClient.get(adminEndpoints.systemConfigs.getByGroup('api'))
